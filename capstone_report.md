...
There was no pre processing required as I took data directly from GroupLens(http://files.grouplens.org/datasets/movielens/ml-latest.zip).
As the recommender system provided by GroupLens is the benchmark and we need to use the same data on both the system.
